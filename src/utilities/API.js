...
export const createUser = async (user) => {
  return fetch('https://bottd-movie-tracker.herokuapp.com/api/users/new', {
    method: 'POST',
    body: JSON.stringify(user),
    headers:{
      'Content-Type': 'application/json'
...
export const removeFavorite = async (movie, user) => {
  return fetch(`https://bottd-movie-tracker.herokuapp.com/api/users/${user.id}/favorites/${movie.id}`, {
    method: 'DELETE',
    body: JSON.stringify({
      movie_id: movie.id,
      user_id: user.id,
...
export const getFavorites = async (user) => {
  const response = await fetch(`https://bottd-movie-tracker.herokuapp.com/api/users/${user.id}/favorites`);
